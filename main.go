...
/*
	updateRotor will update the given (proforma) rotor in place using
	(psudo-random) data generated by the TNT2 encrytption algorithm using
	the proforma rotors and permutators.
*/
func updateRotor(r *rotor.Rotor, left, right chan cryptors.CypherBlock) {
	...
	// Get the next 32 bytes from the uberJC1 algorithm.  This acts as a seed
	// for generating the random data for the rotor.
	var blk cryptors.CypherBlock
	blk.Length = cryptors.CypherBlockBytes
